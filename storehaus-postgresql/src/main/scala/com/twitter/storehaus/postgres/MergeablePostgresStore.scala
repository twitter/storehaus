...
with MergeableStore[K, V2] {
  ...
  override def multiMerge[K1 <: K](kvs: Map[K1, V2]): Map[K1, Future[Option[V2]]] = {
    val result = doUpdate(kvs).run(underlying.client)
    kvs.keySet.iterator.map { key =>
      (key, result.map(_.get(key).map(toV2)))
    }.toMap
  }

  private def doUpdate[K1 <: K](kvs: Map[K1, V2]): DBRequest[Map[K1, V1]] = Kleisli {
    _.inTransaction(
      for{ existingKeys <- underlying.doGet(kvs.keySet) // select elements from store
           _ <- underlying.doUpsert(mergeWithSemigroup(existingKeys, kvs).mapValues(inj).toList)
      } yield existingKeys
    )
  }
