...
object MemcacheLongStore extends NumericInjections {
  ...
  val LongInjection = long2String.andThen(MemcacheStringStore.StringInjection)
...
class MemcacheLongStore(underlying: MemcacheStore) 
  extends ConvertedStore[String, String, ChannelBuffer, Long](underlying)(identity)(MemcacheLongStore.LongInjection)
  with MergeableStore[String, Long] {

  val monoid = implicitly[Monoid[Long]]

  override def merge(kv: (String, Long)) = {
    println("merging %s" format kv)
