 ...
 * If the value stored in the underlying store cannot be converted back to V2,
 * then you will get a Future.exception containing the string "cannot be converted"
 * TODO: we should add a specific exception type here so we can safely filter these cases to
 * Future.None if we so choose.
 */
@deprecated("Use com.twitter.storehaus.ConvertedStore instead", "0.3.1")
class ConvertedStore[K1, -K2, V1, V2](store: Store[K1, V1])(kfn: K2 => K1)
  (implicit inj: Injection[V2, V1])
  extends ConvertedReadableStore[K1, K2, V1, V2](store)(kfn)({ v1: V1 =>
    inj.invert(v1).map(Future.value)
      .getOrElse(Future.exception(new Exception(v1.toString + ": V1 cannot be converted to V2")))
  })
  with Store[K2, V2] {

  override def put(kv: (K2, Option[V2])): Future[Unit] = {
    val k1 = kfn(kv._1)
    val v1 = kv._2.map { inj(_) }
    store.put((k1, v1))
  }

  override def multiPut[K3 <: K2](kvs: Map[K3, Option[V2]]): Map[K3, Future[Unit]] = {
