...
import com.twitter.bijection.{ Bijection, Conversion, Injection, ImplicitBijection }
...
class ConvertedMergeableStore[K1, -K2, V1, V2](store: MergeableStore[K1, V1])(kfn: K2 => K1)
  ...
  override def semigroup: Semigroup[V2] = AlgebirdBijections.semigroupBijection[V2, V1].inverse(store.semigroup)

  override def merge(kv: (K2, V2)): Future[Option[V2]] = {
    val k1 = kfn(kv._1)
    val v1 = bij.bijection(kv._2)
    store.merge((k1, v1)).map(_.map(_.as[V2]))
  }
  ...
  override def multiMerge[K3 <: K2](kvs: Map[K3, V2]): Map[K3, Future[Option[V2]]] = {
    ...
    kvs.keySet.map { k3 => (k3, res(kfn(k3)).map(_.map(_.as[V2]))) }(breakOut)
  }
