...
object ReportingStoreProperties extends Properties("ReportingStore") {
  def newStore[K, V]: JMapStore[K, V] = new JMapStore[K, V]

  class DummyReporter[K, V](val self: Store[K, V])
    ...
    def tracePut(kv: (K, Option[V]), request: Future[Unit]): Future[Unit] = request.unit
    def traceMultiPut[K1 <: K](
        kvs: Map[K1, Option[V]], request: Map[K1, Future[Unit]]): Map[K1, Future[Unit]] =
      request.mapValues(_.unit)
  }
  ...
  property("Put Some/None count matches") = forAll { (inserts: Map[Int, Option[Int]]) =>
    ...
    inserts.foreach(i => wrappedStore.put((i._1, i._2)))
    inserts.values.collect{case Some(b) => b}.size == putSomeCount &&
      inserts.values.collect{case None => 1}.size == putNoneCount
  }
  ...
  property("MultiPut Some/None count matches") = forAll { (inserts: Map[Int, Option[Int]]) =>
    ...
    val wrappedStore = new DummyReporter[Int, Int](baseStore) {
      override def traceMultiPut[K1 <: Int](
          kvs: Map[K1, Option[Int]], request: Map[K1, Future[Unit]]): Map[K1, Future[Unit]] = {
        kvs.mapValues {v =>
          Future {
            v match {
    ...
    inserts.values.collect{case Some(b) => b}.size == multiPutSomeCount &&
      inserts.values.collect{case None => 1}.size == multiPutNoneCount
  }
}
